...
@dataclass
class SuccessBufferConfig(BaseConfig):
    """Configuration for per-prompt success rollout buffer.

    Args:
        max_rollouts_per_prompt (int): Maximum number of successful rollouts stored per prompt.
        sample_old_rollouts_per_update (int): Number of old rollouts to sample per update step.
        store_only_correct (bool): Only store verifier-confirmed correct rollouts.
        deduplicate_exact_tokens (bool): Deduplicate by exact token sequence.
        eviction (str): Eviction strategy: "fifo" or "lowest_logprob".
    """
...
@dataclass
class RolloutSuppressionConfig(BaseConfig):
    """Configuration for success-conditioned rollout suppression loss.

    Args:
        enable (bool): Whether to enable rollout suppression.
        beta (float): Suppression strength coefficient.
        margin (Optional[float]): Negative margin for relu(s_theta - margin).
            None means no margin. Recommended values: -4.0, -6.0, -8.0.
        length_normalize (bool): Whether to length-normalize the anti-logprob.
        buffer (SuccessBufferConfig): Per-prompt success buffer configuration.
        anti_update_freq (int): Run anti-loss update every N normal PPO updates.
            1 = every step. Must be >= 1.
    """
    ...
    buffer: SuccessBufferConfig = field(default_factory=SuccessBufferConfig)
    anti_update_freq: int = 1
...
class AlgoConfig(BaseConfig):
    ...
    # Success-conditioned rollout suppression: pushes model away from old successful trajectories
    rollout_suppression: Optional[RolloutSuppressionConfig] = None
