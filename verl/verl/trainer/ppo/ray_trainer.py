...
from verl.trainer.config import AlgoConfig, RolloutSuppressionConfig
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        # Initialize success-conditioned rollout suppression
        self.rollout_suppression_config = self.config.algorithm.get("rollout_suppression", None)
        if self.rollout_suppression_config is not None:
            rollout_suppression_config = omega_conf_to_dataclass(
                self.rollout_suppression_config, RolloutSuppressionConfig
            )
            self.rollout_suppression_config = rollout_suppression_config
            if rollout_suppression_config.enable:
                from verl.trainer.ppo.success_buffer import PerPromptSuccessBuffer

                self.success_buffer = PerPromptSuccessBuffer(rollout_suppression_config.buffer)
            else:
    ...
    def _build_anti_dataproto(
        self, old_batch_data: dict, anti_meta: dict
    ) -> DataProto:
        """Build a DataProto containing old rollout sequences for a separate anti-loss update."""
        import numpy as np
        from tensordict import TensorDict

        input_ids = old_batch_data["input_ids"]
        attention_mask = old_batch_data["attention_mask"]
        old_rollout_mask = old_batch_data["old_rollout_mask"]

        n_anti = input_ids.shape[0]
        total_len = input_ids.shape[1]
        resp_len = old_rollout_mask.shape[1]
        prompt_len = total_len - resp_len

        # Build batch tensors matching the regular batch structure.
        # All tensors padded to the same dimensions as the anti-data.
        # position_ids: consecutive for real tokens, 0 for padding (left-padded prompts)
        pos_ids = attention_mask.cumsum(dim=-1) - attention_mask  # 0 at padding, then 0, 1, 2, ...
        anti_batch_td = TensorDict(
            {
                ...
                "response_mask": old_rollout_mask,
                "old_log_probs": torch.zeros(n_anti, resp_len),
                "advantages": torch.zeros(n_anti, resp_len),
                "token_level_rewards": torch.zeros(n_anti, resp_len),
                "token_level_scores": torch.zeros(n_anti, resp_len),
            },
            batch_size=n_anti,
        )

        # Build non-tensor batch
        anti_non_tensor = {
            "uid": np.array(old_batch_data["old_rollout_uids"], dtype=object),
            "data_source": np.array(["anti"] * n_anti, dtype=object),
        }

        anti_batch = DataProto(batch=anti_batch_td, non_tensor_batch=anti_non_tensor)
        anti_batch.meta_info["temperature"] = self.config.actor_rollout_ref.rollout.temperature
        anti_batch.meta_info["anti_data"] = anti_meta
        return anti_batch
    ...
    def _update_actor(self, batch: DataProto) -> DataProto:
        ...
        # Pass anti-data (old rollout suppression) through to loss function
        anti_data = batch.meta_info.get("anti_data", None)
        tu.assign_non_tensor(
            batch_td,
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        # --- Success Buffer: record successful rollouts ---
                        if self.success_buffer is not None:
                            from verl.trainer.ppo.success_buffer import SuccessBufferEntry

                            store_only_correct = self.rollout_suppression_config.buffer.store_only_correct
                            for i in range(len(batch)):
                                score = batch.batch["token_level_scores"][i].sum().item()
                                is_correct = score > 0
                                should_store = is_correct if store_only_correct else True

                                if should_store:
                                    prompt_uid = batch.non_tensor_batch["uid"][i]
                                    response_ids = batch.batch["responses"][i].tolist()
                                    response_text = self.tokenizer.decode(
                                        [t for t in response_ids if t != self.tokenizer.pad_token_id],
                                        skip_special_tokens=True,
                                    )
                                    prompt_ids = batch.batch["prompts"][i].tolist()
                                    ...
                                    entry = SuccessBufferEntry(
                                        ...
                                        reward=score,
                                        created_step=self.global_steps,
                                        data_source=batch.non_tensor_batch.get(
                                            "data_source", ["unknown"]
                                        )[i],
                                    )
                                    self.success_buffer.add(entry)
                    ...
                    if self.config.trainer.critic_warmup > self.global_steps:
                        ...
                        # --- Anti-Loss: separate update suppressing old successful rollouts ---
                        if self.success_buffer is not None and self.global_steps % self.rollout_suppression_config.anti_update_freq == 0:
                            with marked_timer("anti_loss", timing_raw, color="purple"):
                                prompt_uids = batch.non_tensor_batch["uid"]
                                old_rollouts = self.success_buffer.sample_old_rollouts(
                                    prompt_uids=list(prompt_uids),
                                    n=self.rollout_suppression_config.buffer.sample_old_rollouts_per_update,
                                )
                                if old_rollouts:
                                    from verl.trainer.ppo.anti_loss import build_old_rollout_batch

                                    old_batch_data = build_old_rollout_batch(
                                        old_rollouts=old_rollouts,
                                        tokenizer=self.tokenizer,
                                        max_response_length=self.config.data.max_response_length,
                                        pad_token_id=self.tokenizer.pad_token_id,
                                    )
                                    anti_meta = {
                                        "old_batch_data": old_batch_data,
                                        "beta": self.rollout_suppression_config.beta,
                                        "margin": self.rollout_suppression_config.margin,
                                        "length_normalize": self.rollout_suppression_config.length_normalize,
                                    }
                                    anti_batch = self._build_anti_dataproto(
                                        old_batch_data, anti_meta
                                    )
                                    anti_output = self._update_actor(anti_batch)
                                    anti_output_metrics = reduce_metrics(anti_output.meta_info["metrics"])
                                    metrics.update(anti_output_metrics)
                ...
                # rollout suppression buffer metrics
                if self.success_buffer is not None:
                    buffer_sizes = [
                        self.success_buffer.get_buffer_size(uid)
                        for uid in batch.non_tensor_batch["uid"]
                    ]
                    metrics["rollout_suppression/buffer_size_mean"] = np.mean(buffer_sizes)
                    metrics["rollout_suppression/buffer_size_max"] = np.max(buffer_sizes)
                    metrics["rollout_suppression/total_entries"] = float(self.success_buffer.total_entries())
